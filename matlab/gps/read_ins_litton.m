function gps = read_ins_litton(litton_fn, param)
% gps = read_ins_litton(litton_fn, param)
%
% Read's raw INS data from Litton 100. The first dataset we received
% was from ATM group for May 3, 2010 Greenland P3.
%
...
%
% File format: ASCII with four fields
% time(seconds of day)  pitch  roll  heading
%
% Example:
%
...
if sum(bad_mask) > 0
  warning('Removing %d records due to repeat time stamps\n', sum(bad_mask));
end
...
gps.lat = zeros(size(gps.gps_time));
gps.lon = zeros(size(gps.gps_time));
gps.elev = zeros(size(gps.gps_time));

gps.pitch= C{2}/180*pi;
gps.roll = C{3}/180*pi;
gps.heading = C{4}/180*pi;
