...
if ~isfield(frames,'frame_idxs')
  ...
  if isempty(param)
    if isfield(frames,'param')
      ...
      error('A param structure must be passed in instead of frames filename to update the old file format. After updating, the filename may be used with frames_load.');
    end
  end
  frames_update(param,[]);
