...
params = read_param_xls(ct_filename_param('snow_param_2011_Greenland_P3.xls'),'20110329_02','post');

params = ct_set_params(params,'cmd.generic',1);
% params = ct_set_params(params,'cmd.frms',[1:10:150,158:300]); % Specify specific frames (or leave empty/undefined to do all frames)
params = ct_set_params(params,'cmd.frms',[]); % Specify specific frames (or leave empty/undefined to do all frames)

param_override.layer_tracker.debug_level = 0;
...
%   track_override.init.search_rng	= [-10:10];
%   track_override.feedthru.time = 1e-5*[-0.0084    0.0471    0.0768    0.1342    0.1749    0.2039];
%   track_override.feedthru.power_dB = [-95.7021  -69.3129  -39.6250  -40.4855  -58.8433  -96.5627];
...
elseif 1
  ...
  track_override.method = 'threshold';
  track_override.threshold_noise_rng = [15e-9 -60e-9 -15e-9];
  ...
  track_override.filter_trim = [60 10];
  track_override.max_rng	= [0 0.5e-9];
  track_override.detrend = '/cresis/snfs1/dataproducts/ct_data/ct_tmp/echogram_stats/snow/2011_Greenland_P3/stats_20110329_01.mat';
  if 0
    track_override.init.method	= '';
  elseif 0
    track_override.init.method	= 'dem';
    track_override.init.dem_offset = 67e-9;
