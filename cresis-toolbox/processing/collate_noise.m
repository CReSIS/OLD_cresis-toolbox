...
%% Input Checks
% =========================================================================
if ~isfield(param.analysis,'simulate_noise')
  param.analysis.simulate_noise = 0;
end
...
stat_dir = ct_filename_out(param,param.analysis.cmd{3}.out_path,'',1);
if ~exist(stat_dir, 'dir')
  fprintf('Empty directory --> run_analysis results (%s)\n',stat_dir);
  return;
end

out_dir = ct_filename_ct_tmp(param,'','stat','fft');
  ...
  for wf_adc = 1:size(param.analysis.imgs{img},1)
    ...
    for b_size = 1:length(dd.stats)
      ...
      for b_cmd = 1:length(dd.stats{b_size})
        ...
        % To simulate Noise data TBD
        if param.analysis.simulate_noise
          [B,A] = butter(2,0.5);
          for idx = 1:size(dd.stats{b_size}{b_cmd},2)
            M = 1000;
            n_sig_sim = filtfilt(B,A,1/sqrt(2)*(randn(fft_len,M) + 1i*randn(fft_len,M)));
            dd.stats{b_size}{b_cmd}(:,idx) = mean(abs(fir_dec(fft(sqrt(BoltzmannConst*290*30e6*2*50)/sqrt(presums) * n_sig_sim),averages(b_cmd))).^2,2)/50;
%             dd.stats{b_size}{b_cmd}(:,idx) = mean(abs(fir_dec(fft(sqrt(BoltzmannConst*290*30e6*2*50)/sqrt(presums(img)) * filtfilt(B,A,1*(randn(fft_len,M)))),averages(b_cmd))).^2,2)/50;
          end
        end
    ...
    mean_power = 10*log10(mean(pow_spec_cmd(2:end,:,adc),1)/fft_len/30e6) + 30;
    ...
    plot(h_axes, 10*log10(pow_spec_cmd(2:end,:,adc)/fft_len/30e6)+30,'LineWidth',3);
    hold(h_axes, 'on');
    ...
    %% Save the plots
    saveas(cur_fig, fullfile(out_dir,sprintf('fft_wf_%d_adc_%d.fig',wf,adc)));
    saveas(cur_fig, fullfile(out_dir,sprintf('fft_wf_%d_adc_%d.jpg',wf,adc)));
