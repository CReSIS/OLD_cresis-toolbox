function out = mle_initialization1(DCM,param,doa_prev_optimum)
...
if isfield(param,'search_type') && strcmpi(param.search_type,'grid')
    ...
    % % % %   Comparing computation time and accuracy of using the previous MLE result
    %to initialize the next stage (one more source)
    ...
    elseif param.Nsig > 1
        ...
        %initilize with previous result of mle (get it from param)
        
        out(:,1) = doa_prev_optimum.';
    end
...
% %     
    ...
    %SRAVYA
    % intilizing last source
    if param.Nsig > length(doa_prev_optimum) && param.Nsig > 1
        clear L max_idx
        for src_idx =length(doa_prev_optimum)+1: param.Nsig
            ...
            % prev_doa = initilize with previous result of mle (get it from param)
