function [array_param,dout] = array_proc(array_param,din,sources_true_all)
...
% CHECK THIS
Nsnap = length(array_param.bin_rng) + length(array_param.rline_rng) + Na + Nb;
    ...
    for binIdx_idx = 1:1:length(array_proc_bin_idxs)
        ...
        switch array_param.method
            ...
            case 7
                ...
                for N_sig_idx = 1:length(Nsig_tmp)
                    
                    array_param.Nsig = Nsig_tmp(N_sig_idx);
                    doa_param.Nsig =  array_param.Nsig;
                    ...
                    for src_idx = 1:array_param.Nsig
                        ...
                        %
                        doa_param.src_limits{src_idx} = [-80 80].*pi/180; % DEBUG
                    end
                ...
                for model_order_method = 0:7
                    ...
                    [sources_number,doa] = sim.model_order_optimal(doa_mle,Nc,Rxx,Nsnap,binIdx, model_order_method);
                    ...
                    doa = doa(1:sources_number);
                    sort_idxs = sort_idxs(1:sources_number);
                    dout.hessian(binIdx,1:sources_number,lineIdx) = HESSIAN(sort_idxs + length(doa)*(sort_idxs-1));
                    ...
                    dout.power(binIdx,1:sources_number,lineIdx)  = P_hat;
                    
                    end
                    
                    
                    dout.Nest_mdl(binIdx,lineIdx) = sources_number;
...
%         suptitle ('OPTIMAL estimators (using MLE)')                   
                 ...
                 % suboptimal methods
                    ...
                    for model_order_method = 1:7
                    
                    clear sources_number doa
                    sources_number = sim.model_order_suboptimal(Nc,Rxx,Nsnap,binIdx, model_order_method);
...
%                         case 0
%                             
%                             model_order_results_suboptimal.MDL.dout  = dout;
%                             model_order_results_suboptimal.MDL.doa_deg  = doa_deg;
%                             
...
%   suptitle ('SUBOPTIMAL Estimators (Eigen based methods)');                   
    ...
    %SRAVYA saving results for a range line 
    
    Nest_slice(:,1) = sources_true_all(:,lineIdx);
    
                    Nest_slice(:,2) = model_order_results_suboptimal.AIC.Nest(:,lineIdx);
                    Nest_slice(:,3) = model_order_results_suboptimal.HQ.Nest(:,lineIdx);
                    Nest_slice(:,4) = model_order_results_suboptimal.MDL.Nest(:,lineIdx);
                    Nest_slice(:,5) = model_order_results_suboptimal.AICc.Nest(:,lineIdx);
                    Nest_slice(:,6) = model_order_results_suboptimal.KICvc.Nest(:,lineIdx);
                    Nest_slice(:,7) = model_order_results_suboptimal.WIC.Nest(:,lineIdx);
              
                   
                   
                    Nest_slice(:,8) = model_order_results.AIC.Nest(:,lineIdx);
                    Nest_slice(:,9) = model_order_results.HQ.Nest(:,lineIdx);
                     Nest_slice(:,10) = model_order_results.MDL.Nest(:,lineIdx);
                    Nest_slice(:,11) = model_order_results.AICc.Nest(:,lineIdx);
                    Nest_slice(:,12) = model_order_results.KICvc.Nest(:,lineIdx);
                    Nest_slice(:,13) = model_order_results.WIC.Nest(:,lineIdx);


                    Nest_slice(:,14) = model_order_results.MDL_sravya.Nest(:,lineIdx);
                                        
                 
  
  Nest_2D{lineIdx} = [[0:6 1:7] ; Nest_slice];
  
  

  
% %   %Comparision of performance of all methods for a single slice  (lineIdx)
% %   figure(1),clf;
% %   plot(1:100,Nest_2D{lineIdx}(2:end,1:7))
% %   legend('true', 'AIC', 'HQ', 'MDL', 'AICc', 'KICvc', 'WIC')
% %   
% %   figure(2),clf;
% %   plot(1:100,Nest_2D{lineIdx}}(2:end,[1 8:14]))
% %   legend('true', 'AIC', 'HQ', 'MDL', 'AICc', 'KICvc', 'WIC','sravya')
  ...
  dout.model_order_results = model_order_results;
