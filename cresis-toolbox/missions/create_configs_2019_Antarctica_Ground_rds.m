...
for freq_idx = [1]
  ...
  config.tg.rg_start_offset = [0 0 0];
  ...
  config.wfs(1).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
  config.wfs(2).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
  config.wfs(3).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
...
%% PingPong Mode
% <4500 m thick ice, 1200 +/- 500 ft AGL
ice_thickness = [4500];
for freq_idx = [1]
  param = default_radar_params_2019_Antarctica_Ground_rds;
  config = param.config;
  ...
  config.tg.staged_recording = {[1 2 3 3]};
  config.tg.altitude_guard = 100*12*2.54/100;
  config.tg.Haltitude = 100*12*2.54/100;
  config.tg.Hice_thick = ice_thickness(freq_idx);
  ...
  config.wfs(1).presums = 128;
  config.wfs(2).presums = 128;
  config.wfs(3).presums = 256;
  config.wfs(4).presums = 256;
  config.tukey = 0.08;
  config.wfs(1).Tpd = 0.5e-6;
  config.wfs(2).Tpd = 3e-6;
  config.wfs(3).Tpd = 10e-6;
  config.wfs(4).Tpd = 10e-6;
  config.wfs(1).tukey = 0.1;
  config.wfs(2).tukey = 0.1;
  config.wfs(3).tukey = 0.1;
  config.wfs(4).tukey = 0.1;
  
  config.wfs(1).name = '0p5us_high';
  config.wfs(2).name = '3us_high';
  config.wfs(3).name = '10us_lefttx';
  config.wfs(4).name = '10us_righttx';
  
  config.wfs(1).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
  config.wfs(1).tx_enable = [1 1 1 1];
  config.wfs(2).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
  config.wfs(2).tx_enable = [1 1 1 1];
  config.wfs(3).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
  config.wfs(3).tx_weights([2 4]) = 0;
  config.wfs(3).tx_enable = [1 0 1 0];
  config.wfs(4).tx_weights = final_DDS_amp{cal_settings_list(freq_idx)};
  config.wfs(4).tx_weights([1 3]) = 0;
  config.wfs(4).tx_enable = [0 1 0 1];
