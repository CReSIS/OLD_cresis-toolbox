...
Hwindow_orig = [1 1 1 1 0 0 0 0]; % Desired window created during transmit calibration
...
% DDS Clock Stabilization/Alignment Oct 8, 2019:
% 1C 1C 1B 1C 1D 1D 1B 1D
% 
% ch8: 1D(14-28)
% ch7: 1B(13-24)
% ch6: 1D(13-28)
...
ice_thickness = [4000];
freq_idx = 1;
param = struct('radar_name','mcords3','num_chan',4,'aux_dac',[255 255 255 255 255 255 255 255],'version','10.0','TTL_prog_delay',650,'fs',1e9/7,'fs_sync',1e9/14,'fs_dds',1e9,'TTL_clock',1e9/14,'TTL_mode',[3e-6 290e-9 -830e-9],'xml_version',2.0,'DDC_freq',0,'DDC_select',0);
param.max_tx = [32000 32000 32000 32000 32000 32000 32000 0]; param.max_data_rate = 80; param.flight_hours = 7; param.sys_delay = 9.68e-6; param.final_tx_mask = final_tx_mask;
...
param.fn = fullfile(base_dir,'survey_mode_10us_3wf_4000mthick.xml');
param.prf = 12000;
...
param.tukey = 0.01;
param.fn = fullfile(calval_dir,'survey_mode_10us_3wf_4000mthick_TTL_CHECK.xml');
write_cresis_xml(param);
% Noise Mode
param.fn = fullfile(base_dir,'survey_mode_10us_3wf_4000mthick_NOISE.xml');
param.tx_weights = DDS_amp;
...
param.fn = fullfile(calval_dir,'survey_mode_10us_3wf_4000mthick_LOOPBACK.xml');
write_cresis_xml(param);
...
param.fn = fullfile(base_dir,'survey_mode_10us_3wf_4000mthick_DECONVOLUTION.xml');
write_cresis_xml(param);
...
param.tg.Hice_thick = 4000;
param.fn = fullfile(base_dir,'image_mode_10us_6wf_4000mhigh_altitude.xml');
param.prf = 10000;
param.presums = [15 15];
param.wfs(1).atten = 0;
param.wfs(2).atten = 0;
...
param.tg.Haltitude = 1250*12*2.54/100;
param.tg.Hice_thick = 4000;
param.fn = fullfile(base_dir,'image_mode_10us_6wf_4000mthick.xml');
param.prf = 12000;
...
param.fn = fullfile(base_dir,'image_mode_10us_6wf_4000mthick_DECONVOLUTION.xml');
write_cresis_xml(param);
...
% freq_idx = 1;
% param = struct('radar_name','mcords3','num_chan',4,'aux_dac',[255 255 255 255 255 255 255 255],'version','10.0','TTL_prog_delay',650,'fs',1e9/7,'fs_sync',1e9/14,'fs_dds',1e9,'TTL_clock',1e9/14,'TTL_mode',[3e-6 290e-9 -830e-9],'xml_version',2.0,'DDC_freq',0,'DDC_select',0);
% param.max_tx = [32000 32000 32000 32000 32000 32000 32000 0]; param.max_data_rate = 500; param.flight_hours = 7; param.sys_delay = 9.68e-6; param.final_tx_mask = final_tx_mask;
% param.max_duty_cycle = 0.12;
% param.create_IQ = false;
% param.tg.staged_recording = false;
% param.tg.altitude_guard = 0*12*2.54/100;
