...
% param.base_dir = 'D:\';
param.base_dir = '/process3_SSD/20150313/mcords/';
...
%   mcords4: 2
%   mcords5: 2 (DDS is 4, but DAQ is 2)
%   (THIS OFTEN NEEDS TO BE SET)
xml_version = 2;
...
param.wf_mapping = [1 2 0 0 0 0 0 0];
...
param.bad_chan_mask = [0 0 1 1 1 1 1 1];
...
Hwindow_desired = [1 1 0 0 0 0 0 0];
% max_DDS_amp = Amplitudes will be normalized to this scalar value
% max_DDS_amp = 15000;
max_DDS_amp = [30000 30000 0 0 0 0 0 0];
...
if strcmpi(param.radar_name,'mcords5') && settings.DDC_Ctrl.DDC_sel.Val >= 0
  %% DDC Enabled
  fs = double(settings.DDC_Ctrl.sampling_freq)*1e6 / 2^(double(settings.DDC_Ctrl.DDC_sel.Val)+1);
  param.pc_param.f0 = double(settings.(config_var).Waveforms(1).Start_Freq(1)) - double(settings.DDC_Ctrl.NCO_freq)*1e6;
  param.pc_param.f1 = double(settings.(config_var).Waveforms(1).Stop_Freq(1)) - double(settings.DDC_Ctrl.NCO_freq)*1e6;
else
  %% No DDC
...
for file_idx = 1:length(fns)
  ...
  for chan = 1:length(param.wf_mapping)
    if param.wf_mapping(chan) ~= 0
      ...
      fprintf('TX %d: %4.2f dB (%4.2f dB)\n', chan, delta_power(chan), ...
        lp(std(ref_power(good_rlines & median_mask))));
      if param.plot_en
