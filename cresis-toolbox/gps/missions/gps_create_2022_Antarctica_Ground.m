% script gps_create_2022_Antarctica_Ground
%
% Makes the GPS files for 2022_Antarctica_Ground field season
...
season_name = '2022_Antarctica_Ground';
...
for idx = 1:length(file_type)
  ...
  if ~isempty(regexpi(gps_source,'arena'))
    ...
    if length(gps.lat) >= 2
      new_gps_time = [gps.gps_time(1)-20, gps.gps_time,gps.gps_time(end)+10];
