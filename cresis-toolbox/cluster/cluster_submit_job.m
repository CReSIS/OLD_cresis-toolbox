...
if strcmpi(ctrl.cluster.type,'torque')
  ...
  if ctrl.cluster.interactive
    cmd = sprintf('qsub -I %s -e %s -o %s -v INPUT_PATH="%s",OUTPUT_PATH="%s",CUSTOM_TORQUE="1",TASK_LIST=''%s''', ...
    ...
    cmd = sprintf('qsub %s -e %s -o %s -v INPUT_PATH="%s",OUTPUT_PATH="%s",CUSTOM_TORQUE="1",TASK_LIST=''%s'' %s  </dev/null', ...
