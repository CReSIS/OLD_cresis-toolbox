function er = iceCond(T,rho,freq,conductivity,eV,Tref)
% er = iceCond(T,rho,freq,conductivity,eV,Tref)
%
% T = temperature in [Kelvin]
% rho = density of ice/snow [g/cm^3]
% freq = frequency in [Hz]
% conductivity = total high-frequency limit conductivity (e.g. measured by
%    DEP), [Siemans per meter]
%    This conductivity includes ice and impurities.
%    Background is hard coded to 9e-6 (so setting conductivity to 9e-6 is like have no impurities)
...
ice_prop = load(fullfile(gRadar.data_support_path,'em_model_data','matsuoka_ice_prop.txt'));

% 2,4,6: Matsuoka's interpolated data
...
% 3,8,10: Johari's data (higher loss)
% A = interp1(ice_prop(:,1),ice_prop(:,3)/1e4,T,'spline','extrap');
% B = interp1(ice_prop(:,1),ice_prop(:,8)/1e5,T,'spline','extrap');
% C = interp1(ice_prop(:,1),ice_prop(:,10),T,'spline','extrap');
...
% Uses exact Debye-relaxation formula (virtually no difference to high frequency
% approximation, even down to 1 MHz).
% e_inf = (3.1884+0.00091*(T-273.15));
% auty = load(fullfile(gRadar.data_support_path,'em_model_data','auty_ice_prop.txt'));
% e_stat = interp1(auty(:,1),auty(:,2),T-273.15,'spline','extrap');
% tao = interp1(auty(:,1),auty(:,4)/1e5,T-273.15,'spline','extrap');
% % Debye-relaxation
% er = e_inf + (e_stat-e_inf) ./ (1+j*2*pi*freq*tao);
% % Infrared
% er = er -j*(B.*(freq/1e9).^C);
