...
//  CT Surface bounds support, Reece Mathews, July 2020
//
...
public:
  ...
  TRWS(const float *image, const size_t *dim_image, const float *at_slope, const float *at_weight,
          ...
          const unsigned int *bounds, const unsigned int *ct_bounds_right, const unsigned int *ct_bounds_left, unsigned int *result)
          : mImage(image), mAT_Slope(at_slope), mAT_Weight(at_weight), mCT_Slope(ct_slope), mCT_Weight(ct_weight),
            mMax_Loops(max_loops), mBounds(bounds), mCT_Bounds_Right(ct_bounds_right), mCT_Bounds_Left(ct_bounds_left), mResult(result) {
            // Set dimensions
            mNt      = dim_image[0];
            mNsv     = dim_image[1];
            mNx      = dim_image[2];
  ...
  for (size_t w = 0; w < mNx; w++) {
    for (size_t h = 0; h < mNsv; h++) {
      ...
      for (size_t d = mBounds[2*w], message_idx = h*mNt + w*mNsv*mNt+mBounds[2*w]; d <= mBounds[2*w+1]; d++, message_idx++) {
        if (h < mCT_Bounds_Right[mNt*w+d] || h > mCT_Bounds_Left[mNt*w+d]) {
          continue;
        }
        // Unary cost
    ...
    for (size_t w_idx = 0; w_idx < mNx; w_idx++) {
      ...
      for (size_t h_idx = 0; h_idx < mNsv; h_idx++) {
        ...
        if (w > 0) {
          ...
          dt(message_sum, &(mMessage_Right[msg_dest_idx]), cur_rbin_start, cur_rbin_stop, dest_rbin_start, dest_rbin_stop, *mAT_Weight, -mAT_Slope[w], &(mCT_Bounds_Right[w*mNt]), &(mCT_Bounds_Left[w*mNt]), h);
        ...
        if (h > 0) {
          ...
          int dest_rbin_stop = mBounds[2*w+1];
          // Binary cost
          dt(message_in, &(mMessage_Down[msg_dest_idx]), cur_rbin_start, cur_rbin_stop, dest_rbin_start, dest_rbin_stop, mCT_Weight[h], mCT_Slope[h+w*mNsv], &(mCT_Bounds_Right[w*mNt]), &(mCT_Bounds_Left[w*mNt]), h);
          // Normalize message so smallest message has a cost of zero
          float min_val = INFINITY;
          for (size_t d = dest_rbin_start, message_idx = msg_dest_idx+dest_rbin_start; d <= dest_rbin_stop; d++, message_idx++) {
        ...
        if (w < mNx-1) {
          ...
          dt(message_in, &(mMessage_Left[msg_dest_idx]), cur_rbin_start, cur_rbin_stop, dest_rbin_start, dest_rbin_stop, *mAT_Weight, mAT_Slope[w], &(mCT_Bounds_Right[w*mNt]), &(mCT_Bounds_Left[w*mNt]), h);
          // Normalize message so smallest message has a cost of zero
          float min_val = INFINITY;
          for (size_t d = dest_rbin_start, message_idx = msg_dest_idx+dest_rbin_start; d <= dest_rbin_stop; d++, message_idx++) {
        ...
        if (h < mNsv-1) {
          ...
          int dest_rbin_start = mBounds[2*w];
          int dest_rbin_stop = mBounds[2*w+1];
          dt(message_in, &(mMessage_Up[msg_dest_idx]), cur_rbin_start, cur_rbin_stop, dest_rbin_start, dest_rbin_stop, mCT_Weight[h], -mCT_Slope[h+w*mNsv], &(mCT_Bounds_Right[w*mNt]), &(mCT_Bounds_Left[w*mNt]), h);
          // Normalize message so smallest message has a cost of zero
          float min_val = INFINITY;
          for (size_t d = dest_rbin_start, message_idx = msg_dest_idx+dest_rbin_start; d <= dest_rbin_stop; d++, message_idx++) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  float *ct_slope = (float *)mxGetData(prhs[arg]);
  ...
  float *ct_weight = (float *)mxGetData(prhs[arg]);
  ...
  unsigned int *ct_bounds_right = (unsigned int *)mxGetData(prhs[arg]);
  ...
  unsigned int *ct_bounds_left = (unsigned int *)mxGetData(prhs[arg]);
  ...
  // Run TRWS algorithm
  TRWS obj(image, dim_image, at_slope, at_weight, ct_slope, ct_weight, *max_loops, bounds, ct_bounds_right, ct_bounds_left, result);
