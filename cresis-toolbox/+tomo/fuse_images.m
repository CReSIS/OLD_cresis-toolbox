      ...
      if trans_bins(end) <= size(mdata{v_img}.Data,1)
        ...
        if doa_method_flag
          New_Theta(:,:,rline) = [Tomo.theta(1:img_bins(1,rline),:,rline); ...
            bsxfun(@times,weights,Tomo.theta(trans_bins,:,rline)) ...
            + bsxfun(@times,difference*(1-weights),mdata{v_img}.Tomo.theta(trans_bins,:,rline)); ...
            difference*mdata{v_img}.Tomo.theta(img_bins(2,rline)+1:end,:,rline)];
          New_Cost(:,rline) = [Tomo.cost(1:img_bins(1,rline),rline); ...
            bsxfun(@times,weights,Tomo.cost(trans_bins,rline)) ...
            + bsxfun(@times,difference*(1-weights),mdata{v_img}.Tomo.cost(trans_bins,rline)); ...
            difference*mdata{v_img}.Tomo.cost(img_bins(2,rline)+1:end,rline)];
          New_Hessian(:,:,rline) = [Tomo.hessian(1:img_bins(1,rline),:,rline); ...
            bsxfun(@times,weights,Tomo.hessian(trans_bins,:,rline)) ...
            + bsxfun(@times,difference*(1-weights),mdata{v_img}.Tomo.hessian(trans_bins,:,rline)); ...
