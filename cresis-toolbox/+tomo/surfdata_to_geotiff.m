...
%% User Settings
surfdata_fn = '/cresis/snfs1/dataproducts/ct_data/rds/2014_Greenland_P3/CSARP_surfData/20140401_03/Data_20140401_03_037.mat';

data_fn = '/cresis/snfs1/dataproducts/ct_data/rds/2014_Greenland_P3/CSARP_CSA_music/20140401_03/Data_20140401_03_037.mat';
...
surf(2).y(surf(2).y<surf(1).y) = surf(1).y(surf(2).y<surf(1).y);

%% Load echogram
mdata = load(data_fn,'theta','Time','GPS_time','param_combine');

% Convert surface from range bins to twtt
ice_surface = interp1(1:length(mdata.Time), mdata.Time, surf(1).y);
ice_bottom = interp1(1:length(mdata.Time), mdata.Time, surf(2).y);
...
[y_bot,z_bot] = tomo.twtt_doa_to_yz(repmat(theta,[1 Nx]),theta(3:end-2),ice_surface(3:end-2,:),3.15,ice_bottom);
[y_surf,z_surf] = tomo.twtt_doa_to_yz(repmat(theta,[1 Nx]),theta(3:end-2),ice_surface(3:end-2,:),3.15,ice_surface);

%% Convert from radar FCS to ECEF
x_plane = zeros(size(y_bot));
y_plane = zeros(size(y_bot));
z_plane = zeros(size(y_bot));
for rline = 1:size(y_bot,2)
  x_plane(:,rline) = mdata.param_combine.array_param.fcs{1}{1}.origin(1,rline) ...
    + mdata.param_combine.array_param.fcs{1}{1}.y(1,rline) * y_bot(:,rline) ...
    + mdata.param_combine.array_param.fcs{1}{1}.z(1,rline) * z_bot(:,rline);
  y_plane(:,rline) = mdata.param_combine.array_param.fcs{1}{1}.origin(2,rline) ...
    + mdata.param_combine.array_param.fcs{1}{1}.y(2,rline) * y_bot(:,rline) ...
    + mdata.param_combine.array_param.fcs{1}{1}.z(2,rline) * z_bot(:,rline);
  z_plane(:,rline) = mdata.param_combine.array_param.fcs{1}{1}.origin(3,rline) ...
    + mdata.param_combine.array_param.fcs{1}{1}.y(3,rline) * y_bot(:,rline) ...
    + mdata.param_combine.array_param.fcs{1}{1}.z(3,rline) * z_bot(:,rline);
end
...
points.lat = points.lat(3:end-2,:);
points.lon = points.lon(3:end-2,:);
points.elev = points.elev(3:end-2,:);
...
if 1
  ...
  hcolor = colorbar;
  set(get(hcolor,'YLabel'),'String','Ice Bottom Elevation (WGS-84,m)')
  ...
  saveas(1,'~/ice_bottom_20140401_03_037.fig');
  saveas(1,'~/ice_bottom_20140401_03_037.jpg');
end
...
if 1
  ...
  hcolor = colorbar;
  set(get(hcolor,'YLabel'),'String','Ice Bottom Elevation (WGS-84,m)')
  saveas(2,'~/ice_bottom_contour_20140401_03_037.fig');
  saveas(2,'~/ice_bottom_contour_20140401_03_037.jpg');
end
...
DEM_geotiff_fn = '~/ice_bottom_DEM_20140401_03_037.tif';
...
csv_fn = '~/ice_bottom_20140401_03_037.csv';
fid = fopen(csv_fn,'w');
fprintf(fid,'X,Y,Elevation,Latitude,Longitude\n');
