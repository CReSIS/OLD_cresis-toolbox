...
% WHAT IT DOES: GRID SEARCH on 3D data with Viterbi Algorithm
tic

%% User Settings
user_settings_viterbi_grid_3D;                                 % (FILE TO EDIT FOR EACH TEST) set up the image sources, references, and the parameter combinations
...
% include in the grid search; they are fixed as constants throughout the
% tests
...
%% data visualization
result_visualize(data_matrix, 'grid', 'Viterbi', '3D', best_parameter_set);       % show a table of the ranked result of rmse from low to high
                                                              % and show a plot for each parameter vs. the error (using the best combinations available)

%% save the result
save_result(data_matrix, '3D', 'Viterbi', Viterbi_param,...
  'grid search', segments_and_frame, best_parameter_set);     % save the test result in the RESULTS folder
