function [hdr,data] = basic_load_fmcw4(fn,param)
% [hdr,data] = basic_load_fmcw4(fn, param)
%
...
%
% fn = '/cresis/snfs1/scratch/yan/2015_NRL_Snow_Radar_data/snow/chan1/snow_01_20150224_142813_02_0002.bin';
% [hdr,data] = basic_load_fmcw4(fn,struct('clk',125e6,'file_version',6));
%
% Authors: John Paden
%
...
% ===================================================================
%% Data Format,entry with * is different from previous snow radar
% ===================================================================
% 32-bit header
% 32-bit EPRI
...
% *16-bit delay of reference signal, 1 delay is equal to 8ns*
% 8-bit presums
% 8-bit bit shift
% 16-bit start
% 16-bit stop
...
% *8-bit (Boolean 1 or 0) tranmitter select, 1=nadir,0=side-looking*
...
% 8-bit (Boolean 1 or 0) ADC input or constant input
% 8-bit (Boolean 1 or 0) raw data or ddc data
% DATA: depending on settings
% If raw data then 16 bit real with size stop-start
% *If DDC data then 32 bit complex (16-bit I and Q) with size (stop-start)/(2^(1+DDC_Filter_Select))*
%
...
if nargout < 2 && ~param.records.en
  ...
  hdr.nadir_or_sidelooking_select = fread(fid,1,'uint8');
...
%     keyboard
  ...
  hdr.delay(rline) = fread(fid,1,'uint16');
  ...
  hdr.nadir_or_sidelooking_select(rline) = fread(fid,1,'uint8');
  ...
  if param.records.en
    ...
    if hdr.DDC_or_raw_select(rline)
      % *Real data, no need to reshape for NRL
...
hdr.nadir_or_sidelooking_select = hdr.nadir_or_sidelooking_select(1:rline);
