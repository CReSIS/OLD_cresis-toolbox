...
wf = 1;
BW = abs(param.radar.wfs(wf).f1-param.radar.wfs(wf).f0) * param.radar.wfs(wf).fmult;
alpha = BW / param.radar.wfs(wf).Tpd;
nz_twtt = param.radar.fs/2/alpha;

nz = floor((agl_twtt+param.radar.wfs(wf).Tsys) / nz_twtt);
interp_nz = isnan(nz);
...
fprintf('%s\tRecommended default nz:\t%d\t%d\n', param.day_seg, default_nz, param.radar.wfs(wf).nyquist_zone);
default_nz = param.radar.wfs(wf).nyquist_zone;

nz(isnan(nz)) = default_nz;
nz(nz<0) = 0;
nz_before_max_threshold = nz;
nz(nz>max_nz) = max_nz;
...
if ~isfield(frames,'nyquist_zone')
  frames.nyquist_zone = NaN*ones(size(frames.frame_idxs));
  frame_changes = true;
end
nz = nz_before_max_threshold;
