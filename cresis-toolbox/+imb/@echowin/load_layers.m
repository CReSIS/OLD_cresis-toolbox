...
elseif strcmpi(obj.eg.layer_source,'layerdata')
  ...
  for frm = obj.eg.frame_idxs(1):obj.eg.frame_idxs(end);
    ...
    for idx=1:length(obj.eg.layer_id)
      obj.eg.layer.x{obj.eg.layer_id(idx)} = cat(2,obj.eg.layer.x{obj.eg.layer_id(idx)},GPS_time); % gps time
      qual = obj.undo_stack.user_data.layer_info(frm).layerData{idx}.quality;
      obj.eg.layer.qual{obj.eg.layer_id(idx)} = cat(2,obj.eg.layer.qual{obj.eg.layer_id(idx)},qual); % quality (integer 1-3)
      twtt = obj.undo_stack.user_data.layer_info(frm).layerData{idx}.value{2}.data;
      obj.eg.layer.y{obj.eg.layer_id(idx)} = cat(2,obj.eg.layer.y{obj.eg.layer_id(idx)},twtt); % twtt
