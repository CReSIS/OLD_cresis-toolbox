...
%dparam.cpu_time = 36000;
%dparam.mem = 250e6;




%cluster_compile(sim_doa_task,[],0,ctrl);
cluster_compile({'sim_doa_task'},ctrl.cluster.hidden_depend_funs,ctrl.cluster.force_compile,ctrl);
...
% ctrl_chain = cluster_set_chain(ctrl_chain,'cluster.cpu_time_mult',4); % duplicate test
% ctrl = cluster_load_chain([],chain_id)
if block
  % Blocking, submit and wait
  fprintf('Submitting %s\n', ctrl.batch_dir);
  ctrl_chain = {{ctrl}};
  ctrl_chain = cluster_set_chain(ctrl_chain,'cluster.cpu_time_mult',1);
