...
physical_constants
if ~isfield(param,'method_mode') || isempty(param.method_mode)
  param.method_mode = 'mle';
end
mle_mode = param.method_mode;
...
%   A = sqrt(1/length(param.y_pc)) * exp(1i*k*(-param.z_pc*cos(theta) + param.y_pc*sin(theta)));
  Pa  = A * inv(A'*A) * A';
%   L = abs(trace((eye(size(Pa))-Pa)*DCM)); % Mohanad (if use this, remove the - from val)
%   L = abs(trace(Pa*DCM)); % Wax
  L = abs(sum(sum(Pa .* DCM.'))); % John
end

if strcmp(mle_mode,'s-mle')
  % Incorporate the a priori pdf if available
  mean_doa = param.apriori.mean_doa;
  cov_mtx_doa  = diag(param.apriori.var_doa);
  L_apriori = -1/2 * log10(det(cov_mtx_doa)) - ((theta.' - mean_doa)' * inv(cov_mtx_doa) * (theta.' - mean_doa));
  L = L + L_apriori;
end
