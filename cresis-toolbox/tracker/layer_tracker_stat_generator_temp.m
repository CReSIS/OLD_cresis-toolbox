...
params_all = [];
idx_segment = 1;
params = read_param_xls(ct_filename_param('rds_param_2014_Greenland_P3.xls'),'20140516_01|20140313_08');
params(idx_segment).cmd.generic = 1;
params(idx_segment).cmd.frms = [1:3];
...
options.season_vs_gps.multiple_figs = false; % choose this option if you separate plots generated for each layer. If set to false, 6 subplots will be plotted in one figure.
...
img_dir = '/cresis/snfs1/scratch/anjali/cluster_tuning/stat_generator_lsm_mult_segs';
...
%             set(gca,'XTickLabel',[param.cmd.frms]);
            ...
            xlim = ([1 length(param.cmd.frms)]);
            title(sprintf('%s Layer: %d',param.day_seg,layer_idx),'Interpreter', 'none');
            percent_error{layer_idx}(hist_idx) = percent_error{layer_idx}(hist_idx)/total_length * 100;
          end
          img_format = 'png';
