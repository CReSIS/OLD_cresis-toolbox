...
%% Track: Fasttime Cross Correlation
...
elseif strcmpi(track.method,'viterbi')
  new_layers = tracker_viterbi(mdata,param);
  new_quality = ones(1,Nx);
elseif strcmpi(track.method,'lsm')
  new_layers = tracker_lsm_track(data,param);
  new_quality = ones(1,Nx);
elseif strcmpi(track.method,'stereo')
  [new_layers,big_matrix] = tracker_stereo(mdata,param);
  new_quality = ones(1,Nx);
elseif strcmpi(track.method,'mcmc')
  new_layers = tracker_mcmc(mdata,param);
  new_quality = ones(1,Nx);
...
for frm_idx = 1:length(param.layer_tracker.frms)
  ...
  tmp_out_fn_name = sprintf('m%03d_%s.mat', method_idx, param.layer_tracker.track.method);
