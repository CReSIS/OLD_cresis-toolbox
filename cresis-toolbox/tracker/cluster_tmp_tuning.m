...
param= temp.param;
res_matrix_surface = zeros(9,16);
res_matrix_bottom = zeros(9,16);
layer_params = [];
idx = 0;
idx = idx + 1;
    ...
    switch param.layer_tracker.track{track_idx}.method
      case 'lsm'
        ...
        for i = 1:16
          num_isnan= 0;
          num_points = 0;
          surf = interp_finite(interp1(track_data.gps_time,track_data(i).twtt,data.GPS_time));
        ...
        for i = 17:32
          bot = interp_finite(interp1(track_data.gps_time,track_data(i).twtt,data.GPS_time));
