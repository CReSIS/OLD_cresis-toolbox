...
for xx=1:2
  ...
  if ( ~exist('surf_peak_idx', 'var') || isempty(surf_peak_idx) )
    surf_idx = surf_closest_idx; %%%#########################
  else
    surf_idx = surf_peak_idx;
  end
  
  % create the elevation_axis
  air_idxs = 1:surf_idx;
  AGL(xx) = dd.Time(surf_idx) *c/2;
  sub_idxs = min(surf_idx+1,Nt):Nt;
  range = [ dd.Time(air_idxs) .*c/2 ; ...
    AGL(xx)+(dd.Time(sub_idxs)-dd.Time(surf_idx)) .*(c/2/sqrt(er_ice))];
  dd.elev_axis = dd.Elevation - range;
  surface_calc(xx) = dd.Elevation - AGL(xx);
  
  % create a time axis to match surfaces
  dd.time_axis = dd.Time - dd.Time(surf_idx);
  
  % Calc powers
  P_est(xx) = 10*log10(1/(4*pi*AGL(xx).^2));
  P_act(xx) = tmp(surf_idx);
