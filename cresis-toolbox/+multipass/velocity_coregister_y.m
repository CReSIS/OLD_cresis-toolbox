...
if ispc
  vel_fn_dir = 'Y:\cbarnett\year_greenland_vv\VY_vel_components';
  % FOR Y-COMPONENT VELOCITIES USE:
  % 'Y:\cbarnett\year_greenland_vv\VY_vel_components'
else
  vel_fn_dir = '/cresis/snfs1/scratch/cbarnett/year_greenland_vv/';
end

example_str = 'Petermann_line1_2011_2014_2015_2017_2018_2019_Y';

if strcmpi(example_str, 'Petermann_line1_2011_2014_2015_2017_2018_2019_Y')
  ...
  vel_fn{1}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic500_2012_2013_vy_v02.1.tif');
  vel_mult{1} = [3];
  vel_fn{2}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2014_2015_vy_v02.1.tif');
  vel_mult{2} = [1];
  vel_fn{3}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2015_2016_vy_v02.1.tif');
  vel_fn{3}{2} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2016_2017_vy_v02.1.tif');
  vel_mult{3} = [1 1];
  vel_fn{4}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2017_2018_vy_v02.1.tif');
  vel_mult{4} = [1];
  % velocity correction for 2018-2019 season uses 2017-2018 data since no
  % NCSIDC velocity map exists for this period of time
  vel_fn{5}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2017_2018_vy_v02.1.tif'); 
  vel_mult{5} = [1];
end
...
if strcmpi(example_str, 'Petermann_line4_2010_2011_2013_2014_2017_Y')
  %% Petermann Line 4 - 2010, 2011, 2013, 2014, 2017 Y COMPONENT
  if ispc
  ...
  vel_fn{1}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic500_2009_2010_vy_v02.1.tif');
  vel_mult{1} = [1];
  vel_fn{2}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic500_2009_2010_vy_v02.1.tif');
  vel_fn{2}{2} = fullfile(vel_fn_dir,'greenland_vel_mosaic500_2012_2013_vy_v02.1.tif');
  vel_mult{2} = [1 1];
  vel_fn{3}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic500_2012_2013_vy_v02.1.tif');
  vel_mult{3} = [3];
  vel_fn{4}{1} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2014_2015_vy_v02.1.tif');
  vel_fn{4}{2} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2015_2016_vy_v02.1.tif');
  vel_fn{4}{3} = fullfile(vel_fn_dir,'greenland_vel_mosaic200_2016_2017_vy_v02.1.tif');
  vel_mult{4} = [1 1 1];
end
  ...
  for sub_pass_idx = pass_idx:length(pass)-1
    for vel_idx = 1:length(vel_fn{sub_pass_idx})
      ...
      if vel_idx == 1 && sub_pass_idx == pass_idx
        ...
        pass(pass_idx).vely = pass(pass_idx).vel;
      else
