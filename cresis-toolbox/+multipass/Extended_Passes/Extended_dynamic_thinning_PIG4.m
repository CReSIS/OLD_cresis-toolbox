%% DYNAMIC VERSION for autoloading all files in Folder - PIG 4
...
% loop will fail.
file_path = cd('C:\Users\c262b531\Documents\scripts\cresis-toolbox\cresis-toolbox\+multipass\QGIS_processed_DIV_SMB\PIG4\');
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'PIG4M_*.csv'));
...
% tidal Amplitudes
XY_data(1).mean_tidal_c = (-0.2181 + (-0.2283))/2;
XY_data(2).mean_tidal_c = (-0.3103 + (-0.3456))/2;
XY_data(3).mean_tidal_c = (-0.1866 + (-0.2199))/2;
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('PIG4M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
