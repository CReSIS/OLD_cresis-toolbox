%% DYNAMIC VERSION for autoloading all files in Folder - Humboldt 1
...
% loop will fail.
file_path = cd('C:\Users\c262b531\Documents\scripts\cresis-toolbox\cresis-toolbox\+multipass\QGIS_processed_DIV_SMB\H1\');
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'H1M_*.csv'));
...
title('Humboldt uncorrected');
xlabel('distance km');
ylabel('elevation m');
legend('2012','2014','2017','Location','southeast');
%% TIDAL CORRECTIONS
...
% tidal Amplitudes
XY_data(1).mean_tidal_c = (-1.286 + (-1.2598))/2;
XY_data(2).mean_tidal_c = (-0.07138 + (-0.009243))/2;
%XY_data(3).mean_tidal_c = (0.01408 + (-0.06483))/2;
XY_data(3).mean_tidal_c = (-1.2665 + (-1.249))/2;

% Extract Ocean Surface with tidal correction, set to 0 tidal level (4km
% for humbolt glacier since profile is not long enough over the ocean
XY_data(1).ocean_surface = XY_data(1).Surf(1008:1299) + XY_data(1).mean_tidal_c;
XY_data(2).ocean_surface = XY_data(2).Surf(1063:1303) + XY_data(2).mean_tidal_c;
XY_data(3).ocean_surface = XY_data(3).Surf(1014:1297) + XY_data(3).mean_tidal_c;

% Extract AT array for each ocean surface, set to 0 tidal level
XY_data(1).AT_ocean = XY_data(1).AT_dist(1008:1299); 
XY_data(2).AT_ocean = XY_data(2).AT_dist(1063:1303);
XY_data(3).AT_ocean = XY_data(3).AT_dist(1014:1297);
...
for i = 1:numel(XY_data)
    ...
    XY_data(i).AT_ocean_5km_mask = XY_data(i).AT_ocean_0 < 4;
...
time_offset.P12 = (235/365); % 2012
time_offset.P13 = (365/365); % 2013
time_offset.P14 = (139/365); % 2014
time_offset.P14_2 = (226/365); % 2014 post echogram
time_offset.P15 = (365/365); % 2015 
time_offset.P16 = (365/365); % 2016
time_offset.P17 = (107/365); % 2017

% total time offset
time_offset.N_years = (time_offset.P12 + time_offset.P13 ...
    + time_offset.P13 + time_offset.P14 + time_offset.P14_2 + ...
...
for i = 1:numel(XY_data)
    ...
    %2011 - 2019 mean
    XY_data(i).SMB_12_17_mean = (XY_data(i).SMB_12t + XY_data(i).SMB_13t + ...
      XY_data(i).SMB_14t + XY_data(i).SMB_14_2t + XY_data(i).SMB_15t + ...
      XY_data(i).SMB_16t + XY_data(i).SMB_17t);
...
figure(222)
plot(XY_data(2).Interp_AT_Dist, XY_data(2).Interp_Div_12_13)
xlabel('Distance (km)');
ylabel('Divergence');
%%
% DIVERGENCE 12_13 FIX-----------------------------------------------------
% define indices of the array holes
XY_data(1).hole_1_12t = (840:926);
XY_data(2).hole_1_12t = (785:926);
XY_data(3).hole_1_12t = (703:927);
% define the mean values of non-hole locations downflow for fixing holes
XY_data(1).Mean_1_12t = mean(XY_data(1).Interp_Div_12_13(1:839));
XY_data(2).Mean_1_12t = mean(XY_data(2).Interp_Div_12_13(1:784));
XY_data(3).Mean_1_12t = mean(XY_data(3).Interp_Div_12_13(1:702));
% Take hole indices
XY_data(1).Interp_Div_12_13(XY_data(1).hole_1_12t) = XY_data(1).Mean_1_12t;
XY_data(2).Interp_Div_12_13(XY_data(2).hole_1_12t) = XY_data(2).Mean_1_12t;
XY_data(3).Interp_Div_12_13(XY_data(3).hole_1_12t) = XY_data(3).Mean_1_12t;
% Test Figure
figure(223)
%plot(XY_data(1).Interp_AT_Dist/1e3, XY_data(1).Interp_Div_12_13)
hold on
%plot(XY_data(2).Interp_AT_Dist/1e3, XY_data(2).Interp_Div_12_13)
plot(XY_data(2).Interp_AT_Dist/1e3, XY_data(2).Interp_Div_12_13)
xlabel('Distance (km)');
ylabel('Divergence');
% DIVERGENCE 14_15 FIX-----------------------------------------------------
%% 5 ICE DIVERGENCE 
for i = 1:numel(XY_data)
    % 2012 - 2014
    ...
    %2011 - 2019 mean
    XY_data(i).div_12_17_mean = (XY_data(i).div_12t + XY_data(i).div_13t + ...
      XY_data(i).div_14t + XY_data(i).div_14_2t + XY_data(i).div_15t + ...
      XY_data(i).div_16t + XY_data(i).div_17t);
end
...
%% ANNUAL MELT RATES 
% 2012 - 2014: Melt, Dynamic Thinning, Mean SMB
% Time Offset
XY_data(1).total_time_offset = time_offset.P12 + time_offset.P13 + ...
    time_offset.P14;

% SMB
XY_data(1).Mean_SMB = (XY_data(i).SMB_12t + XY_data(1).SMB_13t + ...
    XY_data(i).SMB_14t);

% Dynamic Thinning
XY_data(1).Dynamic_component = ((XY_data(1).div_12t + XY_data(1).div_13t + XY_data(1).div_14t)...
    .*XY_data(1).Mean_Thickness);
XY_data(1).Dynamic_component_S150 = ((XY_data(1).div_12t + XY_data(1).div_13t + XY_data(1).div_14t)...
    .*XY_data(1).Mean_Thickness_Smooth150);
XY_data(1).Dynamic_component_S100 = ((XY_data(1).div_12t + XY_data(1).div_13t + XY_data(1).div_14t)...
...
XY_data(1).Hydro_dynamic_component = ((XY_data(1).div_12t + XY_data(1).div_13t + XY_data(1).div_14t)...
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('P4M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
