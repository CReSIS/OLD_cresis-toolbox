%% DYNAMIC VERSION for autoloading all files in Folder - Steensby 1
...
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'S1M_*.csv'));
...
% tidal Amplitudes
XY_data(1).mean_tidal_c = (-0.9486 + -(0.8317))/2;
...
ylabel('elevation (m)');
suptitle('Steensby Line 1 2011-2013')
...
title('Elevation Profiles');
ylabel('elevation (m)');
suptitle('Steensby Line 1 2015-2019')
legend('2015', '2019', 'Location','southeast');
han = axes(fig3,'visible','off');
han.XLabel.Visible ='on';
xlabel(han,'Along Track Distance (km)');

%% Hydrostatic Melt Rates
% use densities of water and ice as 1027 and 917 m/kg^3 
% from Griggs & Bamber, 2017 and Shean, 2017
% https://www.cambridge.org/core/journals/journal-of-glaciology/article/antarctic-iceshelf-thickness-from-satellite-radar-altimetry/3971AAAE7E7E0CA5E8B38533ED2C8AB2
% https://tc.copernicus.org/articles/11/2655/2017/ 
firn_depth = 0;
for i = 1:numel(XY_data)
   XY_data(i).Hydro_thickness = ((XY_data(i).Interp_Surf - firn_depth)*((1027)/(1027-917)));
   XY_data(i).Hydro_bed = (-1)*(XY_data(i).Hydro_thickness - XY_data(i).Interp_Surf);
   XY_data(i).Hydro_melt_total = (XY_data(i+1).Hydro_thickness - XY_data(i).Hydro_thickness) - XY_data(i).Dynamic_component - XY_data(i).Mean_SMB;
   XY_data(i).Hydro_melt_yr = ((XY_data(i+1).Hydro_thickness - XY_data(i).Hydro_thickness) - XY_data(i).Dynamic_component - XY_data(i).Mean_SMB)/XY_data(i).total_time_offset;
   if i > i(end)
        break
   end
end
...
export_melt_rates(i).data_frame = cat(2, XY_data(i).Interp_AT_Dist.', XY_data(i).Interp_Lons.', XY_data(i).Interp_Lats.',...
    ...
    XY_data(i).Mean_SMB.', XY_data(i).Dynamic_component.', XY_data(i).Hydro_thickness.', XY_data(i).Hydro_melt.', XY_data(i).Hydro_melt_yr.');
end
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('S1M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
