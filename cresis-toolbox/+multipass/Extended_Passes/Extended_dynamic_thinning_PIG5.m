%% DYNAMIC VERSION for autoloading all files in Folder - PIG 5
...
% loop will fail.
file_path = cd('C:\Users\c262b531\Documents\scripts\cresis-toolbox\cresis-toolbox\+multipass\QGIS_processed_DIV_SMB\PIG5\');
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'PIG5M_*.csv'));
...
XY_data(2).mean_tidal_c = (-0.02642 + 0.006224)/2;
XY_data(3).mean_tidal_c = (-0.1682 + (-0.1666))/2;
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('PIG5M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
