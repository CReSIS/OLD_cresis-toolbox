%% DYNAMIC VERSION for autoloading all files in Folder - PIG 6
...
% loop will fail.
file_path = cd('C:\Users\c262b531\Documents\scripts\cresis-toolbox\cresis-toolbox\+multipass\QGIS_processed_DIV_SMB\PIG6\');
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'PIG6M_*.csv'));
...
% tidal Amplitudes
XY_data(1).mean_tidal_c = (-0.1999 + (-0.2041))/2;
XY_data(2).mean_tidal_c = (-0.2335 + (-0.2541))/2;
XY_data(3).mean_tidal_c = (-0.1165 + (-0.1334))/2;
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('PIG6M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
