%% DYNAMIC VERSION for autoloading all files in Folder - PIG 7
...
% loop will fail.
file_path = cd('C:\Users\c262b531\Documents\scripts\cresis-toolbox\cresis-toolbox\+multipass\QGIS_processed_DIV_SMB\PIG7\');
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'PIG7M_*.csv'));
...
% tidal Amplitudes
XY_data(1).mean_tidal_c = (-0.1792 + (-0.1820))/2;
XY_data(2).mean_tidal_c = (-0.1276 + (-0.1443))/2;
XY_data(3).mean_tidal_c = (-0.03012 + 0.002650)/2;
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('PIG7M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
