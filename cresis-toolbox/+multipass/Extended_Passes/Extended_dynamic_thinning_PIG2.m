%% DYNAMIC VERSION for autoloading all files in Folder - PIG 2
...
% loop will fail.
file_path = cd('C:\Users\c262b531\Documents\scripts\cresis-toolbox\cresis-toolbox\+multipass\QGIS_processed_DIV_SMB\PIG2_new_bed2\');
myfiles = pwd; 
Velocity_CSV_1 = dir(fullfile(myfiles,'PIG2M_*.csv'));
...
% tidal Amplitudes
XY_data(1).mean_tidal_c = (-0.1724 + (-0.1767))/2;
XY_data(2).mean_tidal_c = (-0.07402 + (-0.1085))/2;
XY_data(3).mean_tidal_c = (0.03746 + 0.002648)/2;

% Extract Ocean Surface with tidal correction, set to 0 tidal level
XY_data(1).ocean_surface = XY_data(1).Surf + XY_data(1).mean_tidal_c;
XY_data(2).ocean_surface = XY_data(2).Surf(5660:6160) + XY_data(2).mean_tidal_c;
XY_data(3).ocean_surface = XY_data(3).Surf(5648:6148) - XY_data(3).mean_tidal_c;

% Extract AT array for each ocean surface, set to 0 tidal level
XY_data(1).AT_ocean = XY_data(1).AT_dist; 
XY_data(2).AT_ocean = XY_data(2).AT_dist(5660:6160);
XY_data(3).AT_ocean = XY_data(3).AT_dist(5648:6148);
...
for i = 1:numel(XY_data)
    ...
    XY_data(i).AT_ocean_5km_mask = XY_data(i).AT_ocean_0 < 7.5;
...
XY_data(1).surf_tidal_elevation_align = XY_data(1).Surf - XY_data(2).mean_ocean_surf_5km + XY_data(1).mean_tidal_c;
...
XY_data(1).bed_tidal_elevation_align = XY_data(1).Bed - XY_data(2).mean_ocean_surf_5km + XY_data(1).mean_tidal_c;
...
for i = 1:3 % define as # of passes or use length(fieldnames(XY_data)) on start 
   ...
   XY_data(i).Ending_clip = XY_data(i).Beginning_clip(XY_data(i).Beginning_clip <= 7.1e4);
...
time_offset.P16_2 = (57/366); % 2016 post echogram
...
for i = 1:numel(XY_data)
    ...
    XY_data(i).SMB_17t = (XY_data(i).Interp_SMB_16*time_offset.P17);
    XY_data(i).SMB_18t = (XY_data(i).Interp_SMB_16*time_offset.P18);
end
%% 5 ICE DIVERGENCE 
...
for i = 1:numel(XY_data)
    ...
    XY_data(i).div_17t = (XY_data(i).Interp_Div_16_17*time_offset.P17);
    XY_data(i).div_18t = (XY_data(i).Interp_Div_16_17*time_offset.P18);
    % 2014 - 2018 mean
...
for j = 1:numel(export_melt_rates)
    name(j).file = append('PIG2M_melt_E_', string(XY_data(j).year),'_',string(XY_data(j+1).year),'.csv');
