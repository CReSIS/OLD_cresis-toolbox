...
for passes_idx = 1: length(passes)
  ...
  if passes_idx == 1 || ~strcmp(passes(passes_idx).day_seg,passes(passes_idx-1).day_seg)
    ...
    param_pass = merge_structs(param_pass, passes(passes_idx).param_override); % PER PASS MERGE
       %dbstop if error
%     dbstop if ~isfield(param,'radar_name')
  ...
  if strcmp(param.combine_passes.input_type,'echo')
    ...
    chan_equal = [];
    for img = 1:length( passes(passes_idx).imgs)
      wf_adc_list = passes(passes_idx).imgs{img};
      for wf_adc = 1:size(wf_adc_list,1)
        ...
        chan_equal ...
          ...
          - metadata{1, passes_idx}.wfs(wf).chan_equal_dB(param_pass.radar.wfs(wf).rx_paths(adc)) )/20) ...
          .* exp(1i*( ...
          param_pass.radar.wfs(wf).chan_equal_deg(param_pass.radar.wfs(wf).rx_paths(adc)) ...
          - metadata{1, passes_idx}.wfs(wf).chan_equal_deg(param_pass.radar.wfs(wf).rx_paths(adc)) )/180*pi);
        Tsys = param_pass.radar.wfs(wf).Tsys(param_pass.radar.wfs(wf).rx_paths(adc));
        Tsys_old = metadata{1, passes_idx}.wfs(wf).Tsys(param_pass.radar.wfs(wf).rx_paths(adc));
        dTsys = Tsys-Tsys_old;
        if dTsys ~= 0
          data{1, passes_idx}{wf, 1} = ifft(bsxfun(@times,fft(data{1, passes_idx}{wf, 1}), ...
            exp(1i*2*pi*metadata{1, passes_idx}.wfs(wf).freq*dTsys)));
        end
        metadata{1, passes_idx}.wfs(wf).chan_equal = chan_equal;
      end
     
      
      % UPDATE WITH THE NEW EQUALIZATION COEFFICIENTS FOR EACH PASS
%       param.array_proc.chan_equal(wf) = chan_equal; % FIND EXACTLY WHERE CHAN_EQUAL WILL BE STORED/APPLIED
%       metadata{1, passes_idx}.wfs(wf).chan_equal = chan_equal{wf};
