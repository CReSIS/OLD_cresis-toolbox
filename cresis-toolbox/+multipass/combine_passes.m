...
for passes_idx = 1: length(passes)
    ...
    if strcmp(param.combine_passes.input_type,'echo')
        ...
        for frm_idx = 1:length(passes(passes_idx).frms)
            ...
            metadata{passes_idx}.lon = [sar_data.(data_field_name)  ifft(bsxfun(@times,fft(sar_data.(data_field_name)),exp(1i*2*pi*sar_data.wfs(wf_equal).freq*dTsys)));
                metadata{passes_idx}.lon ,tmp_data.Longitude];
        ...
        %% Equalization with new Coefficients
        % LOAD NEW EQUALIZATION COEFFICIENTS (saved from Receiver equalization
        % process)
        ...
        for wf_adc = 1:length(length( passes(passes_idx).imgs))
            ...
            chan_equal ...
                  = 10.^((param.radar.wfs(wf_adc).chan_equal_dB(param_pass.radar.wfs(wf_adc).rx_paths(adc)) ...
                - param_sar.radar.wfs(wf_adc).chan_equal_dB(param_pass.radar.wfs(wf_adc).rx_paths(adc)) )/20) ...
                .* exp(1i*( ...
                param.radar.wfs(wf_adc).chan_equal_deg(param_pass.radar.wfs(wf_adc).rx_paths(adc)) ...
                - param_sar.radar.wfs(wf_adc).chan_equal_deg(param_pass.radar.wfs(wf_adc).rx_paths(adc)) )/180*pi);
            Tsys = param.radar.wfs(wf_adc).Tsys_ns(param_pass.radar.wfs(wf_adc).rx_paths(adc));
            Tsys_old = param_sar.radar.wfs(wf_adc).Tsys(param_pass.radar.wfs(wf_adc).rx_paths(adc));
            dTsys = Tsys-Tsys_old;
            if dTsys ~= 0
                data{1, passes_idx}{wf_adc, 1} = ifft(bsxfun(@times,fft(data{1, passes_idx}{wf_adc, 1}),exp(1i*2*pi*metadata{1, passes_idx}.wfs(wf_adc).freq*dTsys)));
                
            end
        end
            
        % UPDATE WITH THE NEW EQUALIZATION COEFFICIENTS FOR EACH PASS
        %{
        % Number of fast-time range bins
        param_sar.radar.wfs(wf_equal).Nt = size(data{1},1); % already defined, use that
        
        param_sar.radar.wfs(wf_equal).df = param_sar.radar.fs / param_sar.radar.wfs(wf_equal).Nt;
        param_sar.radar.wfs(wf_equal).fc = 0.5*(param_sar.radar.wfs(wf_equal).f1+param_sar.radar.wfs(wf_equal).f0);
        param_sar.radar.wfs(wf_equal).freq = param_sar.radar.wfs(wf_equal).fc ...
            + param_sar.radar.wfs(wf_equal).df * ifftshift( -floor(param_sar.radar.wfs(wf_equal).Nt/2) : floor((param_sar.radar.wfs(wf_equal).Nt-1)/2) ).';
        
        chan_equal ...
            = 10.^((param.radar.wfs(wf_equal).chan_equal_dB(param_pass.radar.wfs(wf_equal).rx_paths(adc)) ...
            - param_sar.radar.wfs(wf_equal).chan_equal_dB(param_pass.radar.wfs(wf_equal).rx_paths(adc)) )/20) ...
            .* exp(1i*( ...
            param.radar.wfs(wf_equal).chan_equal_deg(param_pass.radar.wfs(wf_equal).rx_paths(adc)) ...
            - param_sar.radar.wfs(wf_equal).chan_equal_deg(param_pass.radar.wfs(wf_equal).rx_paths(adc)) )/180*pi);
        
        % Correct any changes in Tsys
        Tsys = param.radar.wfs(wf_equal).Tsys_ns(param_pass.radar.wfs(wf_equal).rx_paths(adc));
        Tsys_old = param_sar.radar.wfs(wf_equal).Tsys(param_pass.radar.wfs(wf_equal).rx_paths(adc));
        dTsys = Tsys-Tsys_old;
        if dTsys ~= 0
            % Positive dTsys means Tsys > Tsys_old and we should reduce the
            % time delay to all targets by dTsys.
            % sar_data.(data_field_name) = ifft(bsxfun(@times,fft(sar_data.(data_field_name)),exp(1i*2*pi*sar_data.wfs(wf_equal).freq*dTsys)));
            %             data{pass_idx} = ifft(bsxfun(@times,fft(data{pass_idx}),exp(1i*2*pi*param_sar.radar.wfs(wf_equal).freq*dTsys)));
            % data{pass_idx} = ifft(bsxfun(@times,fft(data{pass_idx}),exp(1i*2*pi*param_sar.radar.wfs(wf).freq*dTsys)));
            %         data{pass_idx} = ifft(bsxfun(@times,fft(data{pass_idx}),exp(1i*2*pi*param_sar.radar.wfs(wf).freq*dTsys)));
            data{1, passes_idx}{wf_equal, 1} = ifft(bsxfun(@times,fft(data{1, passes_idx}{wf_equal, 1}),exp(1i*2*pi*metadata{1, passes_idx}.wfs(wf_equal).freq*dTsys)));
